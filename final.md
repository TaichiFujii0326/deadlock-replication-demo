...
### InnoDBのロック — なぜ循環が生まれるのか

MySQLのストレージエンジンであるInnoDB（データの読み書きを担当するコア部分）は、データの整合性を守るために**行単位でロックをかけます**。

**排他ロック**: INSERTやUPDATEを実行すると、対象の行に「この行は自分が使用中」というロックがかかります。Txが完了するまで、他のTxはこの行を更新できません。

ここで重要なのは、`UPDATE WHERE id != ?` のような範囲条件を実行すると、InnoDBは**条件に合う行だけでなく、インデックスの広い範囲にロックをかけに行く**ということです。具体的には、ネクストキーロック（Next-Key Lock）やギャップロック（Gap Lock）と呼ばれる仕組みにより、「この行だけ」ではなく「この辺り一帯」にロックが広がります。だからこそ相手がINSERTした行も巻き込んで循環待機が起きるのです。
...
前の検証では問題のTxだけを実行して終わりでした。しかし本番では、**問題のTxが走っている最中も、他の通常リクエストがWriterに書き込み続けている**。

そこで条件を変えました。

- **バックグラウンド**: 10 workerが通常の書き込みを継続
- **途中で注入**: 問題のTx（INSERT → API 2秒 → 50,000行 DELETE）を3回実行
...
つまり**3つの負荷が同時に重なっています**。

**負荷1 — ダム決壊**: 問題のTxがロックを保持している間、他のTxがlock waitで待たされます。問題のTxがCOMMITした瞬間、堰き止められていたTxが一斉にCOMMITし、binlogに大量の変更が流れ込みます。

**負荷2 — 通常トラフィックの継続流入**: 問題が解消した後も、Writerには通常のリクエストが来続けます。Replicaは「過去の溜まった分」と「新しく来る分」の両方を処理しなければなりません。

**負荷3 — 並列度のギャップ**: Writerは複数リクエストを並列で処理しますが、Replicaは1つずつ順番に適用するしかありません。一度追いつけなくなると、遅延は雪だるま式に拡大します。

交通渋滞に例えるとこうなります。

> 高速道路で事故が起きて30分間1車線に規制された。事故は処理されたが、溜まった車列は数キロ。規制解除後も通常の車が後ろから来続ける。車列の消化には事故処理よりはるかに長い時間がかかる。

一度流入速度が処理速度を上回ると、**トラフィックが減る時間帯（夜間など）まで遅延は解消されません**。これが本番で数時間のStale Readが続いた理由です。

### 「マルチスレッドレプリケーションで解決できないの？」

MySQL 8.0以降では、Replicaが変更を並列に適用できるマルチスレッドレプリケーション（WRITESET方式など）が利用可能です。「Replicaが一本道だから詰まる」なら、並列にすれば解決するのでは？

結論から言うと、**今回のケースでは効果が限定的**です。

マルチスレッドレプリケーションが並列に適用できるのは、**互いに競合しないTx同士**だけです。同じテーブルの同じ行範囲に対するロック競合がある場合、Replica側でも結局直列に適用するしかありません。今回の問題Txは広範囲にロックをかけるため、通常トラフィックのTxと競合しやすく、並列化の恩恵を受けにくい構造です。

つまり、**Replica側の設定をいくらチューニングしても、Writer側で長時間ロックを保持する設計を直さない限り、ダム決壊は防げません**。
...
※ 「処理全体の時間」はどのパターンも2秒以上（API待ちが含まれるため）になります。本当に重要なのは**ロック保持時間**です。
...
外部APIはデータベース操作の**100〜10,000倍遅い**。この2つを同じTxに入れると、データベースは一瞬で終わるはずの処理のロックを、APIのペースに付き合わされて何千倍も長く保持することになります。

### トランザクションの切り方

```
原則: 1つのTxには「一緒に成功/失敗すべき最小のDB操作」だけを入れる
```

外部通信を挟む時点でTxを分割する。分割すると「APIは成功したのにデータベース更新が失敗した」ケースが生まれますが、`pending` ステータスと定期ジョブで対処できます。一方、分割しないlock waitとレプリカ遅延は**システム全体に波及する問題であり、はるかに対処が困難**です。

### やってはいけない設計

- **Tx内で外部通信**（API／メール／ファイルアップロード）：最も危険。タイムアウト30秒なら最悪30秒ロック保持
- **「念のため」の大きなTx**：関連の薄い処理を1つにまとめると、全体のロック保持時間が伸びる
- **広範囲の論理削除**：`WHERE id != ?` は対象行数が予測しにくく、ロック範囲が膨らみやすい
